...
def main():
    ...
    parser.add_argument('--diameter', required=False, default=19., type=float, 
    ...
    output_args.add_argument('--save_model_dir', type=str, help="directory to save model")
    # training settings
    ...
    training_args.add_argument('--SGD', action='store_true', help="whether to use SGD instead of RAdam")
            ...
            if args.train:
                cpmodel_path = model.train(images, labels, train_files=image_names,
                                        ...
                                        #    save_path=os.path.realpath(args.dir), save_every=args.save_every,
                                           save_path=os.path.realpath(os.path.join(args.save_model_dir, os.path.realpath(test_dir).split("/")[-1])), save_every=args.save_every,
                                           save_each=args.save_each,
                                           rescale=rescale,n_epochs=args.n_epochs,
                                           batch_size=args.batch_size, omni=args.omni, SGD=args.SGD)  # custom change (SGD flag)
